...
def create_assessment(data: AssessmentInputSchema, db: Session = Depends(get_db)):
    ...
    assessment = Assessment(
        ...
        # Safety Triage V2 Specific Fields
        dizziness=bool(data.dizziness) if data.dizziness is not None else False,
        fainting=bool(data.fainting) if data.fainting is not None else False,
        shortness_of_breath=bool(data.shortness_of_breath) if data.shortness_of_breath is not None else False,
        rapid_pad_saturation=bool(data.rapid_pad_saturation) if data.rapid_pad_saturation is not None else False,
        flooding_gushing=bool(data.flooding_gushing) if data.flooding_gushing is not None else False,
        large_blood_clots=bool(data.large_blood_clots) if data.large_blood_clots is not None else False,
        pregnancy_possible=bool(data.pregnancy_possible) if data.pregnancy_possible is not None else False,
        sudden_severe_pelvic_pain=bool(data.sudden_severe_pelvic_pain) if data.sudden_severe_pelvic_pain is not None else False,
        one_sided_pelvic_pain=bool(data.one_sided_pelvic_pain) if data.one_sided_pelvic_pain is not None else False,
        shoulder_tip_pain=bool(data.shoulder_tip_pain) if data.shoulder_tip_pain is not None else False,
        fever_chills=bool(data.fever_chills) if data.fever_chills is not None else False,
        unable_to_keep_fluids=bool(data.unable_to_keep_fluids) if data.unable_to_keep_fluids is not None else False,
        bleeding_between_periods=bool(data.bleeding_between_periods) if data.bleeding_between_periods is not None else False,
        bleeding_after_sex=bool(data.bleeding_after_sex) if data.bleeding_after_sex is not None else False,
